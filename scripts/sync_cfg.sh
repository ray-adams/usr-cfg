...
# Script Path: <git_root>/scripts/sync_usr_cfg.sh
...
tohka_s3() {
    ...
    mkdir -p "${working_dir}/tohka-s3/btop/"
    mkdir -p "${working_dir}/tohka-s3/dunst/"
    mkdir -p "${working_dir}/tohka-s3/imv/"
    ...
    ## btop
    echo "${green} Syncing btop configuration file...${nc}"
    rsync -qu \
        "${XDG_CONFIG_HOME}/btop/btop.conf" \
        "${working_dir}/tohka-s3/btop/"

    ## imv
    echo "${green} Syncing imv configuration file...${nc}"
    rsync -qu \
        "${XDG_CONFIG_HOME}/imv/config" \
        "${working_dir}/tohka-s3/imv/"
